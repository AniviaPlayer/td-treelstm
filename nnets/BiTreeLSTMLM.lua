...
local BiTreeLSTMLM = torch.class('BiTreeLSTMLM', 'BModel')
...
function BiTreeLSTMLM:__init(opts)
  self.opts = opts
  self.name = 'BiTreeLSTMLM'
  self.rnnCount = 5
  self:print( 'build BiTreeLSTMLM ...' )
  ...
  self:print( 'build BiTreeLSTMLM done!' )
end

function BiTreeLSTMLM:createLSTM(x_t, c_tm1, h_tm1, nin, nhid)
...
function BiTreeLSTMLM:createDeepLSTMContext(opts)
...
function BiTreeLSTMLM:createDeepLSTM(opts)
  local emb = nn.LookupTable(opts.nivocab, opts.nin)
  local x_t = nn.Identity()()
  local s_tm1 = nn.Identity()()
...
function BiTreeLSTMLM:createNetwork(opts)
  local emb = nn.LookupTable(opts.nivocab, opts.nin)
...
function BiTreeLSTMLM:initModel(opts)
  self.embeddings = {}
  self.hiddenStates = {}
...
function BiTreeLSTMLM:trainBatch(x, y, lc, lc_mask, sgdParam)
...
function BiTreeLSTMLM:validBatch(x, y, lc, lc_mask)
  if self.opts.useGPU then
    x = x:cuda()
    y = y:cuda()
    lc = lc:cuda()
...
function BiTreeLSTMLM:getHiddenStates(x, lc, lc_mask)
  if self.opts.useGPU then
    x = x:cuda()
    -- y = y:cuda()
    lc = lc:cuda()
...
function BiTreeLSTMLM:fpropStep(act, x_t, s_tm1, lcs)
  local x_t_ = x_t
  local y_t_ = torch.Tensor({0})
  if torch.type(x_t) == 'number' then
    x_t_ = torch.LongTensor({x_t})
...
function BiTreeLSTMLM:disableDropout()
  for i = 1, self.rnnCount do
...
function BiTreeLSTMLM:enableDropout()
  for i = 1, self.rnnCount do
