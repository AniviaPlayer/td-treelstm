...
-- currently only support TreeLSTMNCELM
local Reranker = torch.class('TreeLSTMLMReranker')
...
function Reranker:__init(modelPath, useGPU)
  ...
  elseif opts.model == 'BiTreeLSTMNCE' then
    self.rnnlm = BiTreeLSTMNCELM(opts)
  else
    error('currently only support TreeLSTMNCELM')
  end
...
function Reranker:rerank(testFile, outFile, batchSize)
  ...
  local logp_sents = {}
  local dataIter
  if self.rnnlm.name:starts('BiTree') then
    dataIter = Reranker.createBatchBidirectional(testFile, batchSize)
  else
    dataIter = Reranker.createBatch(testFile, batchSize)
  end
  
  local cnt = 0
  for x, y, lchild, lc_mask in dataIter do
    -- yPred | size: (seqlen*bs, nvocab)
    local _tmp, yPred
    if self.rnnlm.name:starts('BiTree') then
      _tmp, yPred = self.rnnlm:validBatch(x, y, lchild, lc_mask)
    else
      _tmp, yPred = self.rnnlm:validBatch(x, y)
    end
...
local function main()
  ...
  print(opts)
  local reranker = TreeLSTMLMReranker(opts.modelPath, opts.useGPU)
