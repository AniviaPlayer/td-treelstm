...
local TreeLM_Dataset = torch.class('TreeLM_Dataset')

function TreeLM_Dataset:__init(datasetPath)
...
function TreeLM_Dataset:getVocabSize()
  return self.vocab.nvocab
end

function TreeLM_Dataset:getTrainSize()
  return self.trainSize
end

function TreeLM_Dataset:getValidSize()
  return self.validSize
end

function TreeLM_Dataset:getTestSize()
  return self.testSize
end

function TreeLM_Dataset:toBatch(xs, ys, batchSize)
...
function TreeLM_Dataset:createBatch(label, batchSize)
  ...
  return function()
    if istart <= N then
      ...
      return self:toBatch(xs, ys, batchSize)
    end
  end
end

function TreeLM_Dataset:close()
  self.h5in:close()
end
